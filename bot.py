...
# Card generation
# ---------------------
def draw_centered_text(draw, center_x, center_y, text, font, fill="black", x_offset=0, y_offset=0):
...
def generate_card(rewards, scratched):
    ...
    try:
        font = ImageFont.truetype("arial.ttf", 120)  # big readable font
    except:
        font = ImageFont.load_default()

    # Custom x-offsets per square
    x_offsets = [15, 5, 8, 10, 15, 15, 15, 15]
        ...
        if scratched_box:
            ...
            if reward == "10k":
                for offset in range(1,4):
                    draw_centered_text(draw, center_x-offset, center_y, reward, font, fill=(255,215,0), x_offset=x_offsets[i])
                    draw_centered_text(draw, center_x+offset, center_y, reward, font, fill=(255,215,0), x_offset=x_offsets[i])
                    draw_centered_text(draw, center_x, center_y-offset, reward, font, fill=(255,215,0), x_offset=x_offsets[i])
                    draw_centered_text(draw, center_x, center_y+offset, reward, font, fill=(255,215,0), x_offset=x_offsets[i])

            # Draw main prize text with individual x-offset
            draw_centered_text(draw, center_x, center_y, reward, font, fill="black", x_offset=x_offsets[i])
