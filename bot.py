...
VANILLA_POOL = [str(x) for x in range(100, 50001, 100)]  # 100 to 50k
...
# Card generation
# ---------------------
def draw_centered_text(draw, rect_x, rect_y, rect_w, rect_h, text, font, x_offset=0, y_offset=0, fill="black"):
    bbox = draw.textbbox((0,0), text, font=font)
    text_w = bbox[2] - bbox[0]
    text_h = bbox[3] - bbox[1]
    center_x = rect_x + rect_w / 2
    center_y = rect_y + rect_h / 2
    draw.text((center_x - text_w/2 + x_offset, center_y - text_h/2 + y_offset), text, font=font, fill=fill)
...
def generate_card(rewards, scratched):
    ...
    x_offsets = [15, 5, 8, 10, 15, 15, 15, 15]  # custom offsets per square

    for i, scratched_box in enumerate(scratched):
        if scratched_box:
            rect_x, rect_y = RECT_COORDS[i]
            reward = rewards[i]

            # Gold glow for 10k
            if reward == "10k":
                for offset in range(1,4):
                    draw_centered_text(draw, rect_x, rect_y, rect_width, rect_height, reward, font,
                                       fill=(255,215,0), x_offset=x_offsets[i]-offset, y_offset=-offset)
                    draw_centered_text(draw, rect_x, rect_y, rect_width, rect_height, reward, font,
                                       fill=(255,215,0), x_offset=x_offsets[i]+offset, y_offset=offset)

            draw_centered_text(draw, rect_x, rect_y, rect_width, rect_height, reward, font, x_offset=x_offsets[i], fill="black")
