...
def generate_card(rects, revealed):
    ...
    rect_width, rect_height = 180, 80  # adjust for your template
    for i, rect in enumerate(rects):
        x, y = RECT_COORDS[i]
        if revealed[i]:
            # "Scratch" gray rectangle
...
class ScratchView(View):
    def __init__(self, rects, card_type):
        ...
        self.message = None  # stores original card message
        for i in range(8):
...
async def create_scratch_card(interaction, card_type):
    ...
    # Send card and store the message in the view
