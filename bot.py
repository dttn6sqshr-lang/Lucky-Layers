...
STAFF_ROLE_ID = 1474517835261804656  # staff role ID
...
# ---------------------
# PRIZE GENERATION (weighted, granular)
# ---------------------
def generate_rewards(card_type):
    ...
    if card_type == "Sprinkle":
        ...
        prize_indices = random.sample(range(8), 2)
        prizes = PRIZES["Sweet"]
        weights = [5,4,3,2,1,1][:len(prizes)]
        for i in prize_indices:
            rewards[i] = random.choices(prizes, weights=weights, k=1)[0]
        ...
        vanilla_options = list(range(100,50001,50))
...
# ---------------------
# DYNAMIC HEART & BOW DRAW
# ---------------------
def draw_heart(size=60):
    img = Image.new("RGBA", (size,size))
    d = ImageDraw.Draw(img)
    # simple heart shape (triangle + circles)
    d.polygon([(size*0.5,size*0.9),(size*0.1,size*0.5),(size*0.9,size*0.5)], fill=(255,0,0,255))
    d.ellipse([size*0.1,size*0.1,size*0.5,size*0.5], fill=(255,0,0,255))
    d.ellipse([size*0.5,size*0.1,size*0.9,size*0.5], fill=(255,0,0,255))
    return img

def draw_bow(size=20):
    img = Image.new("RGBA", (size,size))
    d = ImageDraw.Draw(img)
    # simple horizontal bow
    d.rectangle([0,size//3,size,size*2//3], fill=(255,192,203,255))
    return img
