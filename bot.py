...
# Adjust for your Canva PNG. Default font is small, so text is slightly offset.
RECT_COORDS = [
...
def draw_prize(draw, x, y, prize):
    ...
    offset_x = len(prize) * 2  # tweak if needed
    draw.text((x - offset_x, y), prize, fill="black", font=ImageFont.load_default())
...
def generate_card(rects, revealed):
    ...
    for i, rect in enumerate(rects):
        if revealed[i]:
            x, y = RECT_COORDS[i]
            draw_prize(draw, x, y, rect['prize'])
...
async def create_scratch_card(interaction, card_type):
    ...
    await interaction.response.send_message(file=discord.File("scratch_result.png"),
                                            view=view,
                                            content=f"🍬 {card_type.capitalize()} Scratch Card! Click the rectangles to scratch!")
