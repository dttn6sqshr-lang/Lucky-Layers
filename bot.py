...
# ---------------------
def draw_text_centered(draw, box_x, box_y, box_width, box_height, text, font_size=150, fill="black"):
    try:
...
def generate_card(rewards, scratched):
    # Base card
    img = Image.new("RGBA", (1800, 1300), (255, 255, 255, 255))
    draw = ImageDraw.Draw(img)

    # Draw scratch boxes
    for i, (x, y) in enumerate(RECT_COORDS):
        color = (200, 200, 200)  # gray boxes
        draw.rectangle([x, y, x + BOX_WIDTH, y + BOX_HEIGHT], fill=color, outline=(0,0,0), width=3)
        if scratched[i]:
            draw_text_centered(draw, x, y, BOX_WIDTH, BOX_HEIGHT, rewards[i], font_size=150, fill="black")
