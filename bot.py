...
def generate_card(rects, revealed):
    ...
    rect_width, rect_height = 180, 80  # adjust to match your gray rectangles
    for i, rect in enumerate(rects):
        x, y = RECT_COORDS[i]
        if revealed[i]:
            # Draw slightly transparent white overlay on top of gray rectangle
            draw.rectangle([x, y, x + rect_width, y + rect_height], fill=(255,255,255,230))
            # Draw prize text
...
class ScratchView(View):
    def __init__(self, rects, card_type):
...
async def create_scratch_card(interaction, card_type):
    ...
    view = ScratchView(rects, card_type)
