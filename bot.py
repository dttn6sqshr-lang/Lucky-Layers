...
def generate_card(rewards, scratched):
    """Generate scratch card with larger, centered text directly on boxes."""
    img = Image.open("scratch_template.png")
    draw = ImageDraw.Draw(img)

    # Use a TrueType font (change to any cute font if you like)
    try:
        font = ImageFont.truetype("arial.ttf", 32)  # Bigger size
    except:
...
class ScratchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Remove button so it disappears
