...
# Draw centered text
# ---------------------
def draw_text_centered(draw, box_x, box_y, box_width, box_height, text, font_size=50, fill="black"):
    try:
...
# ---------------------
# Background with gradient & stars/hearts
# ---------------------
...
def generate_card(rewards, scratched):
    ...
    draw_text_centered(draw, 0, 10, CARD_WIDTH, 60, title_text, font_size=50, fill="black")
    # Draw boxes
    for i, (x,y) in enumerate(RECT_COORDS):
        draw.rounded_rectangle([x, y, x+BOX_WIDTH, y+BOX_HEIGHT], radius=20, fill=(230,230,230))
        if not scratched[i]:
            overlay = Image.new("RGBA", (BOX_WIDTH, BOX_HEIGHT), (192,192,192,200))
            img.alpha_composite(overlay, (x, y))
        if scratched[i]:
            draw_text_centered(draw, x, y, BOX_WIDTH, BOX_HEIGHT, rewards[i], font_size=50, fill="black")
    img.save("card.png")
    return "card.png"
...
class ScratchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await asyncio.sleep(0.5)  # simulate scratch effect
...
async def create_card(interaction, user, command):
    ...
    cards[msg.id] = {"user": user.id, "rewards": rewards, "scratched": scratched, "giver_name": interaction.user.name}
