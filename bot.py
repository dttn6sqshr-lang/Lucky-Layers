...
VANILLA_POOL = [
    str(x) for x in list(range(100,51000,100))
]  # 3 prizes, increments 100 → 50k
...
def generate_card(rewards, scratched):
    ...
    for i, scratched_box in enumerate(scratched):
        if scratched_box:
            ...
            cy = y + (rect_height - text_height) / 2 - 2  # slightly adjust if needed
