...
class ScratchButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for alpha in [210,150,100,50,0]:
            img = create_background()
            draw = ImageDraw.Draw(img)
            # Draw all boxes
            for i,(bx,by) in enumerate(RECT_COORDS):
                draw.rounded_rectangle([bx,by,bx+BOX_WIDTH,by+BOX_HEIGHT], radius=18, fill=(235,235,235))
                if card["scratched"][i] or i==self.index:
                    font_size = 42 if card["rewards"][i] in SWEET_POOL else 36
                    draw_text_centered(draw, bx, by, BOX_WIDTH, BOX_HEIGHT, card["rewards"][i], font_size=font_size)
                else:
                    overlay = Image.new("RGBA", (BOX_WIDTH,BOX_HEIGHT), (180,180,180,210))
                    img.alpha_composite(overlay, (bx,by))
            # Overlay current panel
